...
function generateTwColorClasses(
  globalKey: string,
  twAttributes: { [key: string]: any }
) {
  let classes = {};
...
function kebabToNested(obj: Record<string, any>): Record<string, any> {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (i === parts.length - 1) {
        if (typeof value === "object") {
          currentObj[part] = kebabToNested(value);
        } else {
          currentObj[part] = value.toString().toLowerCase();
        }
      } else {
